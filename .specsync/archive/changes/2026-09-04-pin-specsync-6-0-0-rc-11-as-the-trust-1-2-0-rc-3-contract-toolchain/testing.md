...
- `tests/test.sh` resolver cases accept default `6.0.0-rc.11` with no
...
- `scripts/validate.py` fails unless the nested SpecSync action SHA pairs with
  binary `6.0.0-rc.11` in both `action.yml` and `release.yml`.
- `fledge lanes run verify` (fmt, lint, test) and
  `specsync check --force --strict` pass on this tree with the verified
  rc.11 binary first on PATH.
- `python3 scripts/release_channel.py v1.2.0-rc.3` reports promote=false for
  channel `v1`.
