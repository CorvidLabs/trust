...
- [x] Pin SpecSync 6.0.0-rc.11 in `action.yml` (default, description, nested tag object).
- [x] Update `DEFAULT_SPECSYNC_VERSION` and `validate.py` pairing.
- [x] Pin 6.0.0-rc.11 in `.github/workflows/release.yml` jobs.
- [x] Update the resolver default in `tests/test.sh`; keep mirror rules.
- [x] Set `plugin.toml` to 1.2.0-rc.3 and match README install tag.
...
- [x] Run `fledge lanes run verify` and strict SpecSync check under rc.11.
